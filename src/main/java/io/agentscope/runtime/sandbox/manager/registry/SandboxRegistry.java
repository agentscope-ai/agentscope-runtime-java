...
public class SandboxRegistry {
    ...
    public static void registerCustomType(String typeName, String imageName) {
        ...
        // Create a basic config for the custom type
        // Note: We can't use SandboxType enum for custom types, so we use BASE as placeholder
        SandboxConfig config = new SandboxConfig.Builder()
    ...
    /**
     * Unregister a custom sandbox type
     * 
     * @param typeName The custom type name to unregister
     * @return true if unregistered successfully, false if not found
     */
    public static boolean unregisterCustomType(String typeName) {
        if (typeName == null) {
            return false;
        }
        
        SandboxConfig removed = customTypeRegistry.remove(typeName.toLowerCase());
        DynamicSandboxType.removeCustomType(typeName);
        
        if (removed != null) {
            logger.info("Unregistered custom sandbox type: name=" + typeName);
            return true;
        }
        return false;
    }
    ...
    public static void initializeDefaults() {
        // Register default sandbox types with their images
        register(SandboxType.BASE, 
                "agentscope-registry.ap-southeast-1.cr.aliyuncs.com/agentscope/runtime-sandbox-base:latest");
        
        register(SandboxType.FILESYSTEM, 
                "agentscope-registry.ap-southeast-1.cr.aliyuncs.com/agentscope/runtime-sandbox-filesystem:latest");
        
        register(SandboxType.BROWSER, 
                "agentscope-registry.ap-southeast-1.cr.aliyuncs.com/agentscope/runtime-sandbox-browser:latest");
        
        register(SandboxType.TRAINING, 
                "agentscope-registry.ap-southeast-1.cr.aliyuncs.com/agentscope/runtime-sandbox-appworld:latest");
        
        register(SandboxType.APPWORLD, 
                "agentscope-registry.ap-southeast-1.cr.aliyuncs.com/agentscope/runtime-sandbox-appworld:latest");
        
        register(SandboxType.BFCL, 
                "agentscope-registry.ap-southeast-1.cr.aliyuncs.com/agentscope/runtime-sandbox-bfcl:latest");
        
        register(SandboxType.WEBSHOP, 
                "agentscope-registry.ap-southeast-1.cr.aliyuncs.com/agentscope/runtime-sandbox-webshop:latest");
        
        logger.info("Initialized default sandbox registrations: " + getRegisteredCount() + " types");
    }
}
