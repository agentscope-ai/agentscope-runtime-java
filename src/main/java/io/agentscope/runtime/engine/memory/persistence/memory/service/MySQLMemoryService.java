...
public class MySQLMemoryService implements MemoryService {
    ...
    Logger logger = Logger.getLogger(MySQLMemoryService.class.getName());
    ...
    public CompletableFuture<Boolean> health() {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                logger.severe("MySQL memory service health check failed" + e.getMessage());
                return false;
            }
        });
    }
    ...
    public CompletableFuture<Void> addMemory(String userId, List<Message> messages, Optional<String> sessionId) {
        return CompletableFuture.runAsync(() -> {
            try {
                ...
                logger.info("Successfully added " + messages.size() + " memories to MySQL, user: " + userId + ", session: " + sessionIdValue);

            } catch (Exception e) {
                logger.severe("Failed to add memory to MySQL" + e.getMessage());
                ...
                List<MemorySimilarity> similarities = allMemories.stream()
                        ...
                        .map(memory -> {
                            try {
                                ...
                                logger.warning("Failed to calculate similarity, memory ID: " + memory.getId());
...
//                    .filter(ms -> ms.similarity > 0.1) // Filter low similarity results
                ...
                logger.info("MySQL embedding-based memory search completed, user: " + userId + ", query: " + queryText + ", candidates: " + similarities.size() + ", after deduplication: " + deduplicated.size() + ", returned: " + results.size());

                return results;

            } catch (Exception e) {
                logger.severe("MySQL memory search failed" + e.getMessage());
                return Collections.emptyList();
            }
        });
    }
    ...
    public CompletableFuture<List<Message>> listMemory(String userId, Optional<Map<String, Object>> filters) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                logger.info("MySQL list memory completed, user: " + userId + ", page: " + pageNum + ", page size: " + pageSize + ", result count: " + results.size());

                return results;

            } catch (Exception e) {
                logger.severe("MySQL list memory failed" + e.getMessage());
                return Collections.emptyList();
            }
        });
    }
        ...
        return CompletableFuture.runAsync(() -> {
            try {
                if (sessionId.isPresent()) {
                    ...
                    logger.info("Deleted user session memory, user: " + userId + ", session: " + sessionId.get());
                } else {
                    memoryRepository.deleteByUserId(userId);
                    logger.info("Deleted all user memory, user: " + userId);
                }
            } catch (Exception e) {
                logger.severe("MySQL delete memory failed" + e.getMessage());
    ...
    public CompletableFuture<List<String>> getAllUsers() {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                logger.info("MySQL get all users completed, user count: " + users.size());
                return users;
            } catch (Exception e) {
                logger.severe("MySQL get all users failed" + e.getMessage());
                return Collections.emptyList();
            }
        });
    }
    ...
    private String serializeMessageContent(List<MessageContent> content) {
        try {
            ...
            logger.warning("Failed to serialize message content" + e.getMessage());
            return "[]";
        }
    }
    ...
    private String serializeMetadata(Map<String, Object> metadata) {
        ...
        try {
            ...
            logger.warning("Failed to serialize metadata" + e.getMessage());
            return null;
        }
    }
    ...
    private List<MessageContent> deserializeMessageContent(String contentJson) {
        try {
            ...
            logger.warning("Failed to deserialize message content" + e.getMessage());
            return Collections.emptyList();
        }
    }
    ...
    private Map<String, Object> deserializeMetadata(String metadataJson) {
        try {
            ...
            logger.warning("Failed to deserialize metadata" + e.getMessage());
            return null;
        }
    }
    ...
    private String serializeEmbedding(List<Double> embedding) {
        try {
            ...
            logger.warning("Failed to serialize embedding" + e.getMessage());
            return null;
        }
    }
    ...
    private List<Double> deserializeEmbedding(String embeddingJson) {
        try {
            ...
            logger.warning("Failed to deserialize embedding" + e.getMessage());
            return Collections.emptyList();
        }
    }
