...
will package the application into a Docker image automatically. For customization, use `deployer.yml`. You can also control whether to push to a registry or deploy to Kubernetes via `pushToRegistry` and `deployToK8s`.
